...
export const useGetConsensusProposalsByName = (network: Network, nameFragment: string | undefined) => {
  ...
  const textMatcher = nameFragment
    ? (proposal: generated.Proposal): boolean => {
        if (proposal.handler && proposal.handler.includes(nameFragment)) {
          return true
        }

        if (proposal.cancels && getCancelTitle(proposal.cancels).includes(nameFragment)) {
          return true
        }

        return (
          !!proposal.parameters_change &&
          getParameterChangeTitle(proposal.parameters_change).includes(nameFragment)
        )
      }
    : () => false
