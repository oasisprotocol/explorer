      ...
      {transactions?.map(transaction => (
        <SubPageCard title={t('transaction.header')} key={`${transaction.round}_${transaction.index}`}>
          <StyledDescriptionList titleWidth="200px">
            ...
            <dt>{t('common.table.type')}</dt>
            <dd>
              <RuntimeTransactionLabel method={transaction.method!} />
            </dd>

            <dt>{t('common.table.from')}</dt>
            <dd>{transaction.sender_0}</dd>

            <dt>{t('common.table.to')}</dt>
            <dd>{transaction.to}</dd>

            <dt>{t('common.table.txnFee')}</dt>
            <dd>{t('common.table.valueInRose', { value: transaction.fee_amount })}</dd>

            <dt>{t('common.table.value')}</dt>
            <dd>{t('common.table.valueInRose', { value: transaction.amount })}</dd>
